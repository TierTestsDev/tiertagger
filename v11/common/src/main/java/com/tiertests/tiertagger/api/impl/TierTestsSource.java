...
public class TierTestsSource extends TierSource {
            ...
            try {
                ...
                if (userElement != null && !userElement.isJsonNull()) {
                    ...
                    if (discordIdEl != null && !discordIdEl.isJsonNull()) {
                        ...
                        fetchPeakTiers(playerData, discordId);
                    }
                }
    ...
    private void fetchPeakTiers(PlayerData playerData, long discordId) {
        for (Map.Entry<GameMode, Tier> entry : playerData.getTiers().entrySet()) {
            ...
            try {
                ...
                TierTaggerCommon.LOGGER.debug("[PeakTiers] Fetching: {}", getBase() + endpoint);
                String historyData = makeRequest("GET", endpoint, null);
                if (historyData == null) {
                    TierTaggerCommon.LOGGER.debug("[PeakTiers] Null response for {} / {}", discordId, mode.name());
                    continue;
                }

                TierTaggerCommon.LOGGER.debug("[PeakTiers] Response for {}: {}", mode.name(),
                        historyData.substring(0, Math.min(200, historyData.length())));

                JsonObject historyResponse = JsonParser.parseString(historyData).getAsJsonObject();
                if (historyResponse.isJsonNull() || !historyResponse.get("success").getAsBoolean()) {
                    TierTaggerCommon.LOGGER.debug("[PeakTiers] Non-success response for {}", mode.name());
                    continue;
                }

                JsonArray historyArray = historyResponse.getAsJsonArray("data");
                if (historyArray == null || historyArray.isEmpty()) {
                    TierTaggerCommon.LOGGER.debug("[PeakTiers] Empty data array for {}", mode.name());
                    continue;
                }
                ...
                if (peak != null) {
                    TierTaggerCommon.LOGGER.debug("[PeakTiers] Peak for {}: {}", mode.name(), peak.name());
                    playerData.addPeakTier(mode, peak);
                }
            } catch (Exception e) {
                TierTaggerCommon.LOGGER.error("[PeakTiers] Error fetching history for {} / {}", discordId, mode.name(), e);
            }
        }
    }
